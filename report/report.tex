...
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
  \item \code{exp/c-muon}: cautious mask~\cite{cmuon} (sign-agreement gate) on the polar-express output. Conceptually free---one bitmask, no extra matmul---but did not move loss at $n=8$.
  \item \code{exp/adamuon}: AdaMuon~\cite{adamuon} element-wise variance adaptation overlaid on NorMuon. The bf16 variance buffer added memory traffic; loss flat, wall slightly worse.
  \item \code{exp/per-head-normuon}: redistribute NorMuon's per-axis EMA step-size budget across heads on the QK bank. No measurable signal.
  \item \code{exp/hyperball-adam}: pin \code{lm\_head} Frobenius norm at its initial value (replacing weight decay). Mild loss regression.
  \item \code{exp/ademamix}: AdEMAMix~\cite{ademamix} two-EMA Adam variant for the embedding/scalar bank. The slow EMA needs many more steps than this 1500-step regime allows; losing.
  \item \code{exp/dion}, \code{exp/scion-adam-leg}, \code{exp/schedule-free-adam}: each replaced or augmented an optimizer leg; none beat the existing Muon+NorMuon+Adam mix.
  \item \code{exp/sr-bf16-adam}: store Adam moments in bf16 with stochastic rounding~\cite{srbf16}. Halves optimizer-state memory and reduces all-gather traffic. We saw a small wall-clock win but a $\sim$0.001-nat loss regression that ate the gain.
  \item \code{exp/dsv4-newton-schulz}: swap Polar Express for the DeepSeek-V4 hybrid 8+2 Newton-Schulz schedule~\cite{deepseek_v4}. Net loss regression at iso-step.
\end{itemize}

\subsection{Schedule and curriculum}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
  \item \code{exp/adam-warmup}, \code{exp/adam-warmup-10}: short Adam-only LR warmup (50 / 10 steps). Inconsistent at $n=8$.
  \item \code{exp/muon-lr-floor}: split Adam/Muon cooldown floors (0.15 / 0.80). No signal vs.\ master's shared 0.15 floor.
  \item \code{exp/cwd-acceleration}: split Cautious Weight Decay into separate acceleration and decay coefficients. Flat.
  \item \code{exp/power-scheduler}: power-law cooldown~\cite{power_scheduler} replacing the linear decay. Shape changes the time spent near peak LR but did not help in this regime.
  \item \code{exp/seq-length-curriculum} and \code{exp/seq-curriculum-batch-bump}: shorter \code{train\_max\_seq\_len} on early stages, optionally with a stage-1 batch bump. The wall-clock gain we expected from cheaper varlen attention was eaten by recompiles and a slight loss regression.
  \item \code{exp/leaky-relu-iter-sweep}: \code{NUM\_SCHEDULED\_ITERATIONS} env override for sweeping. Infra change, not a result.
\end{itemize}

\subsection{Attention and value-stream changes}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
  \item \code{exp/qk-gain}: per-(layer, head) learnable post-QK-norm gain. Identity-init, but the optimizer found no useful per-head temperature beyond YaRN's existing scale.
  \item \code{exp/ssmax}: SSMax~\cite{ssmax} log-context attention scaling. Theoretically composes with the YaRN window schedule; empirically flat.
  \item \code{exp/long-window-gqa}: GQA-by-averaging on long-window layers (3 and 10), 3:1 K/V $\to$ Q grouping. After fixing a bug where averaging happened \emph{before} rotary/value-embedding mixing, the corrected version was loss-neutral but gave us no wall-clock either (averaged tensors still have full per-head shape).
  \item \code{exp/fox-gate}: FoX data-dependent forget-gate bias on long-window layers. Bf16-cast and doc/window mask issues; final version flat.
  \item \code{exp/selective-attention}: parameter-free selective attention~\cite{selective_attn} on layer 10 only. After fixing three correctness bugs (acausal cumsum direction, missing doc mask, missing window mask), the materialized $(T,T)$ path made the layer too slow; loss signal alone could not justify the regression.
  \item \code{exp/attn-sink}: per-(layer, head) learnable attention sink (DeepSeek-V4 \S2.3.3~\cite{deepseek_v4}). Loss neutral.
  \item \code{exp/register-tokens}: 4 learnable BOS-position offsets~\cite{registers}. Composes with the existing BOS-alignment recipe but added no loss headroom.
  \item \code{exp/rotating-dim-sweep}: parameterize the RoPE partial-rotation dim (32, 48, 64, 80, 96). The default $head\_dim/2 = 64$ remained best.
\end{itemize}

\subsection{Architectural and capacity changes}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
  \item \code{exp/dyt}: Dynamic Tanh~\cite{dyt} at pre-attn and pre-MLP norms. Flat to slightly negative.
  \item \code{exp/xielu}: xIELU activation~\cite{xielu} with per-layer learnable $\alpha_p,\alpha_n$, eventually with a fused Triton kernel. Same loss as ReLU\(^2\) at slightly worse wall-clock.
  \item \code{exp/leaky-relu-spectral-init}: LeakyReLU($\alpha$)\(^2\) MLP plus an overtone power-law spectral \code{lm\_head} init. Two ideas in one branch; signal too noisy at $n=8$ to justify either.
  \item \code{exp/hourglass-ffn}: shrink MLP inner dim from $4d$ to $2d$. Wall-clock improvement, loss regression, no net.
  \item \code{exp/progressive-depth}: gate top layers off early in training, phase in later. The compiled graph still ran the gated layers at full cost (\code{torch.compile} doesn't dead-code-eliminate buffers), so we measured loss only; even there it was flat.
  \item \code{exp/hmid-injection}: zero-init mid-network state injection into later layers. Flat.
  \item \code{exp/per-token-resid-lambda}, \code{exp/backout-mlp-gate}: per-token sigmoid gates around residual lambdas / backout. Both no-op-init; both flat.
  \item \code{exp/mhc}: minimal $n_{hc}=2$ manifold-constrained hyper-connections, zero-init via lane-0 dominance. Flat.
  \item \code{exp/mudd-residual}: per-token gated dense skips into layers 9 and 10 (residual channel). Not promising on its own---but, kept warm; it would resurface (\S\ref{sec:after}).
\end{itemize}

\subsection{Systems and infrastructure}
\begin{itemize}[leftmargin=1.2em,itemsep=0pt]
  \item \code{exp/cuda-graphs}: capture inductor in \code{reduce-overhead} mode. Compile-time win, no end-to-end speedup once warm.
  \item \code{exp/lawa-wsm}: disable LR cooldown and merge checkpoints at final eval. Not robust at $n=8$; a small fluke can flip the average.
  \item \code{exp/opt-overlap-investigation}: probe pipeline overlap between Muon and Adam steps. Diagnostic only.
\end{itemize}
